...
public class MainQuestPanel : BasePanel
{
    ...
    private void Start()
    {
        ...
        Button_All.onClick.AddListener(() => { AnimateAndHighlight(Button_All); DisplayQuestList("没有主线任务", q => true); });
        Button_CanStart.onClick.AddListener(() => { AnimateAndHighlight(Button_CanStart); DisplayQuestList("没有可开启的主线任务", q => q.state == QuestState.CAN_START); });
        Button_InProgress.onClick.AddListener(() => { AnimateAndHighlight(Button_InProgress); DisplayQuestList("没有进行中的主线任务", q => q.state == QuestState.IN_PROGRESS); });
        Button_CanFinish.onClick.AddListener(() => { AnimateAndHighlight(Button_CanFinish); DisplayQuestList("没有可完成的主线任务", q => q.state == QuestState.CAN_FINISH); });
        Button_Finished.onClick.AddListener(() => { AnimateAndHighlight(Button_Finished); DisplayQuestList("没有已完成的主线任务", q => q.state == QuestState.FINISHED); });

        // 默认选中第一个
        AnimateAndHighlight(Button_All);
        DisplayQuestList("没有主线任务", q => true);
    }
